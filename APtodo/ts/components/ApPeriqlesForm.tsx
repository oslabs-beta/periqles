...
import PeriqlesField from './ApPeriqlesField';
import {introspect, fieldsArrayGenerator} from './ApPeriqlesFunctions';
...
const PeriqlesForm = ({
  ...
  const handleSubmit = (e, fields): void => {
    ...
    if (environment) {
      ...
      // actual invocation of addUser useMutation hook; if passing variables must be passed inside of a variables object
      useMutation({ variables })
      .then(resp => {
        if (callbacks?.onSuccess) callbacks.onSuccess(resp); // useMutation hook returns mutation response fields on the data property of the response object
      })
      .catch(err => {
        if (callbacks?.onFailure) callbacks.onFailure(err); // if onFailure callback provided, invote on useMutation error
      })
    }
  };
